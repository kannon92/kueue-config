...
type ConfigSpec struct {
	...
	// config is the desired configuration for Kueue
	// +required
	Kueue Configuration `json:"config"`
}
...
type ConfigStatus struct {
	...
	// Important: Run "make" to regenerate code after modifying this file
	// desiredConfigReached detects if config map has been synced
	DesiredConfigReached bool `json:"desiredConfigReached,omitempty"`
}
...
type WaitForPodsReady struct {
	...
	// +kubebuilder:validation:XValidation:rule="!self.enable",message="blockAdmission can only be set if enabled is set to true"
